  ...
  armsBack: [
    {
      ...
      label: 'Lat Pulldown',
      hasWeight: true,
      kcalPerRep: (bw, load) => 0.006 * (load + 0.1 * bw),
    },
    {
      id: 'shoulderPress',
    ...
    },
    {
      ...
      label: 'Biceps Curl',
      hasWeight: true,
      kcalPerRep: (bw, load) => 0.004 * (load + 0.1 * bw),
    },
    {
      id: 'pushUp',
      label: 'Push-up',
      hasWeight: false,
      kcalPerRep: (bw) => 0.007 * (0.66 * bw),
    },
    {
      id: 'pullUp',
      label: 'Pull-up',
      hasWeight: false,
      kcalPerRep: (bw) => 0.008 * (1.0 * bw),
    },
  ],
  coreAbs: [
    {
      ...
      label: 'Abdominal',
      hasWeight: true,
      kcalPerRep: (bw, load) => 0.003 * (load + 0.2 * bw),
    },
    {
      id: 'sitUp',
      label: 'Sit-up',
      hasWeight: false,
      kcalPerRep: (bw) => 0.005 * (0.4 * bw),
    },
  ],
};
const EXERCISE_MAP = {
  armsBack: EXERCISE_GROUPS.armsBack.reduce((acc, exercise) => {
    acc[exercise.id] = exercise;
    return acc;
  }, {}),
  coreAbs: EXERCISE_GROUPS.coreAbs.reduce((acc, exercise) => {
    acc[exercise.id] = exercise;
    return acc;
  }, {}),
};
...
function isWorkoutDay(date) {
  return WORKOUT_DAYS.has(date.getUTCDay());
}
...
function readDayData(dateStr) {
  ...
  const armsBack = normalizeExercises(stored.armsBack || {}, 'armsBack');
  const coreAbs = normalizeExercises(stored.coreAbs || {}, 'coreAbs');
  return {
    cycling: {
...
function normalizeExercises(group, groupKey) {
  ...
  list.forEach((item, index) => {
    ...
    if (exerciseId && EXERCISE_MAP[groupKey] && EXERCISE_MAP[groupKey][exerciseId]) {
      selected[exerciseId] = {
        reps: item.reps || '',
        weight: item.weight || '',
      };
...
function ensureDayData(dateStr) {
  ...
  if (!day.armsBack) day.armsBack = { exercises: [] };
  if (!day.coreAbs) day.coreAbs = { exercises: [] };
  if (!Array.isArray(day.armsBack.exercises)) day.armsBack.exercises = [];
  if (!Array.isArray(day.coreAbs.exercises)) day.coreAbs.exercises = [];
  return day;
}
...
function buildExerciseChips(groupKey, groupData) {
  const list = EXERCISE_GROUPS[groupKey] || [];
  const selected = groupData && groupData.selected ? groupData.selected : {};
  return list
    .map((exercise) => {
      const pressed = Boolean(selected[exercise.id]);
      return `<button class="chip" data-exercise="${exercise.id}" aria-pressed="${pressed}">${exercise.label}</button>`;
    })
    .join('');
}
...
function buildDayCard(dateStr, index) {
  ...
  card.innerHTML = `
    ...
    <div class="section">
      <div class="section-title">Arms / Back</div>
    ...
    <div class="section">
      <div class="section-title">Core / Abs</div>
...
function renderExerciseList(card, group, data) {
  ...
  if (!list) return;
  list.innerHTML = '';
  const exercises = EXERCISE_GROUPS[group] || [];
...
function renderSchedule() {
  ...
  const extraPastDays = (state.settings.extraPastWeeks || 0) * 14;
  const extraFutureDays = (state.settings.extraFutureWeeks || 0) * 28;
  const baseRangeStart = addDays(today, -7);
  const baseRangeEnd = addDays(today, 14);
  const rawStart = addDays(today, -(7 + extraPastDays));
  const rangeStart = startDate && rawStart.getTime() < startDate.getTime() ? startDate : rawStart;
  const rangeEnd = addDays(today, 14 + extraFutureDays);
...
function getWeightForDate(dateStr) {
  ...
  const entries = Object.entries(state.metrics)
    ...
    .filter((entry) => !startDate || entry.date.getTime() >= startDate.getTime());

  if (!entries.length) return null;
  entries.sort((a, b) => a.date.getTime() - b.date.getTime());

  if (target) {
    const before = entries.filter((entry) => entry.date.getTime() <= target.getTime());
...
function calculateStrengthEstimate(dayData, bodyWeight) {
  const armsEstimate = estimateExerciseGroup(dayData.armsBack, 'armsBack', bodyWeight);
  const coreEstimate = estimateExerciseGroup(dayData.coreAbs, 'coreAbs', bodyWeight);
  return armsEstimate + coreEstimate;
}

function estimateExerciseGroup(groupData, groupKey, bodyWeight) {
  if (!groupData) return 0;
  const exercises = EXERCISE_GROUPS[groupKey] || [];
  ...
  exercises.forEach((exercise) => {
    ...
    total += reps * exercise.kcalPerRep(bodyWeight, load);
  });
...
function hasAnyActivity(dayData) {
  ...
  const reps = totalReps(dayData.armsBack) + totalReps(dayData.coreAbs);
...
function handleScheduleClick(event) {
  ...
  if (exerciseId !== undefined) {
    ...
    if (existingIndex >= 0) {
      ...
      const exerciseDef = EXERCISE_MAP[group] ? EXERCISE_MAP[group][exerciseId] : null;
